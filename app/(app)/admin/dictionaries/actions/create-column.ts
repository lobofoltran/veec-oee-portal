...
export async function createColumnAction(input: ColumnFormInput) {
  ...
  try {
    await prisma.dictionaryColumn.create({
      data: {
        tableId: parsed.data.dictionaryId,
    ...
    try {
      ...
      console.warn("createColumnAction revalidate", revalidateError);
    }
    return { ok: true };
  } catch (error) {
    console.error("createColumnAction", error);
    return { ok: false, formError: "Não foi possível criar o rascunho da coluna." };
  }
}
